...
public class DataBaseHandler extends SQLiteOpenHelper{
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        String CREATE_ACCOUNTS_TABLE = String.format("CREATE TABLE %s " +
                                       ...
                                       ACCOUNTS_NAME,
                                       ACCOUNTS_BALANCE, 
                                       ACCOUNTS_USER, ACCOUNTS_NAME, ACCOUNTS_USER, TABLE_USERS,
                                       USERS_NAME);
        ...
        String CREATE_TRANSACTIONS_TABLE = String.format("CREATE TABLE %s" +
                ...
                TABLE_TRANSACTIONS, TRANS_USER, TRANS_ACCNAME, TRANS_CAT, 
                TRANS_AMOUNT, TRANS_ISDEPOSIT, TRANS_USERTIME, 
                TRANS_USER, TRANS_ACCNAME, 
    ...
    public ArrayList<Account> getAccounts(Profile user) {
        ...
        while (!cursor.isAfterLast()) {
            Account currentAccount = new Account(cursor.getFloat(1), cursor.getString(0));
    ...
    public void addTransaction(Profile user, Account account, Transaction transaction) {
        ...
        String whereClause = String.format("%s=? AND %s=?", ACCOUNTS_USER, ACCOUNTS_NAME);
        db.update(TABLE_ACCOUNTS, account_vals, whereClause, new String[] {user.getUsername(), account.getName()});
        db.close();
    }
	
	public ArrayList<Transaction> getTransactions(Profile user, Account account) {
        SQLiteDatabase db = this.getReadableDatabase();
        String whereClause = String.format("%s=? AND %s=?", TRANS_USER, TRANS_ACCNAME);
        String[] fields = {TRANS_CAT, TRANS_AMOUNT, TRANS_ISDEPOSIT, TRANS_USERTIME, "rowid"};
        Cursor cursor = db.query(TABLE_TRANSACTIONS, fields, whereClause, new String[]{user.getUsername(), account.getName()},
                null, null, null);
        cursor.moveToFirst();
        ...
        while (!cursor.isAfterLast()) {
            ...
            Transaction newTransaction = new Transaction(amount, isDeposit, category, userTime);
